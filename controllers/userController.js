...
const login = async (req, res) => {
  try {
    ...
    if (!email) {
      return res.status(400).json({ message: 'Username not found', code: 'Please fill username' });
    }

    if (!password) {
      return res.status(400).json({ message: 'Password not found', code: 'Please fill password' });
    }
